...
if prametric_learning:
    ...
    for eval_id in test_eval_ids:
        ...
        io.mesh_io.point_data[f'test_bc_{eval_id}'] = np.array(full_displ_bc_vec).reshape((fe_model.GetNumberOfNodes(), 3))
        io.mesh_io.point_data[f'test_U_FOL_bc_{eval_id}'] = np.array(UVW_test[eval_id]).reshape((fe_model.GetNumberOfNodes(), 3))
    ...
    for eval_id in train_eval_ids:
        ...
        io.mesh_io.point_data[f'train_bc_{eval_id}'] = np.array(full_displ_bc_vec).reshape((fe_model.GetNumberOfNodes(), 3))
        io.mesh_io.point_data[f'train_U_FOL_bc_{eval_id}'] = np.array(UVW_train[eval_id]).reshape((fe_model.GetNumberOfNodes(), 3))
    ...
    for eval_id in FE_eval_ids:
        ...
        unknown_dofs = jnp.zeros(num_unknowns)
        full_displ_bc_vec = mechanical_loss_3d.GetFullDofVector(bc_train_nodal_value_matrix[eval_id,:],unknown_dofs)
        io.mesh_io.point_data[f'bc_{eval_id}'] = np.array(full_displ_bc_vec).reshape((fe_model.GetNumberOfNodes(), 3))
        FE_UVW = np.array(fe_solver.SingleSolve(bc_train_nodal_value_matrix[eval_id],jnp.zeros(3*fe_model.GetNumberOfNodes())))  
        io.mesh_io.point_data[f'U_FE_bc_{eval_id}'] = FE_UVW.reshape((fe_model.GetNumberOfNodes(), 3))
else:
    num_unknowns = mechanical_loss_3d.GetNumberOfUnknowns()
    unknown_dofs = jnp.zeros(num_unknowns)
    ...
    io.mesh_io.point_data['bc'] = np.array(full_displ_bc_vec).reshape((fe_model.GetNumberOfNodes(), 3))
    io.mesh_io.point_data["U_FOL"] = np.array(FOL_UVW).reshape((fe_model.GetNumberOfNodes(), 3))
    FE_UVW = np.array(fe_solver.SingleSolve(bc_train_nodal_value_matrix,jnp.zeros(3*fe_model.GetNumberOfNodes())))  
    io.mesh_io.point_data["U_FE"] = np.array(FE_UVW).reshape((fe_model.GetNumberOfNodes(), 3))

io.mesh_io.write(os.path.join(case_dir, "pr_le_box.vtu"))
