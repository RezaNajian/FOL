...
class TestThermal2D(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model_info = create_2D_square_model_info_thermal_dirichlet(L=1,N=11,T_left=1.0,T_right=0.9,T_bottom=1.0,T_top=1.0)
        ...
        self.fol = FiniteElementOperatorLearning("first_fol",self.fourier_control,[self.thermal_loss],[10],
                                                "swish",working_directory=self.test_directory)
        
        self.coeffs_matrix,self.K_matrix = create_random_fourier_samples(self.fourier_control,200)

    def test_compute(self):
        self.fol.Initialize()
        self.fol.Train(loss_functions_weights=[1],X_train=self.coeffs_matrix[:100,:],batch_size=1,num_epochs=200,
        ...
        T_FEM = np.array(self.fe_solver.SingleSolve(self.K_matrix[1],np.zeros(self.K_matrix[-1].shape)))
        ...
        plot_mesh_vec_data(1,[self.K_matrix[1,:],T_FOL,T_FEM,pointwise_err],["K","T_FOL","T_FEM","rel err %"],file_name=os.path.join(self.test_directory,"FOL-KT-dist.png"),show=True)
